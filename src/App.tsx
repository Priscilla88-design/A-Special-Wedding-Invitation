...
const GoogleIcon = () => (
  <svg className="w-5 h-5" viewBox="0 0 24 24">
    ...
    <path
      fill="#FBBC05"
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22c-.66-1-1.04-2.12-1.04-3.26c0-1.14.38-2.26 1.04-3.26l-2.85-2.22C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94z"
    />
    <path
      fill="#EA4335"
...
const LoginScreen = () => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-merlot flex items-center justify-center p-6 text-ivory">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(163,41,41,0.15)_0%,rgba(131,21,21,0.4)_100%)] opacity-80" />
      <motion.div 
        initial={{ opacity: 0, y: 30, scale: 0.95 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
        className="parchment-card p-12 md:p-20 text-center rounded-sm max-w-md w-full relative z-10"
      >
        <div className="absolute top-4 left-4 right-4 bottom-4 border border-gold/20 pointer-events-none rounded-sm" />
        ...
        <p className="text-slate italic mb-12 text-sm leading-relaxed">
          Please sign in with Google to access Sophie & John's wedding registry, RSVP, and guest details.
        </p>
        
        <button 
          onClick={handleLogin}
          disabled={loading}
          className="w-full bg-white text-slate shadow-sm border border-sand/30 py-4 uppercase tracking-[0.25em] text-[11px] font-bold hover:bg-ivory hover:shadow-md hover:border-gold/50 transition-all flex items-center justify-center gap-3 disabled:opacity-50 cursor-pointer rounded-sm"
        >
          {loading ? (
            <span className="text-slate/60 animate-pulse">Signing in...</span>
          ) : (
            <>
              <GoogleIcon />
        ...
        <p className="mt-8 text-[9px] uppercase tracking-widest text-slate/40">Authorized Guests Only</p>
      </motion.div>
    </div>
  );
};
